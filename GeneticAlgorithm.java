...
public class GeneticAlgorithm {
    private static final String GENES = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOP"
    ...
    public static void main(String[] args) {
        ...
        while (!found) {
            population.sort((ind1, ind2) -> Integer.compare(ind1.getFitness(), ind2.getFitness()));
            if (population.get(0).getFitness() <= 0) {
                found = true;
                break;
            }
            List<GeneticAlgorithm> newGeneration = new ArrayList<>();
            int elitismCount = (10 * POPULATION_SIZE) / 100;
            newGeneration.addAll(population.subList(0,elitismCount));
            int offspringCount = (90 * POPULATION_SIZE) /100;
            Random random = new Random();
            for (int i = 0; i < offspringCount; i++) {
                int indexParent1 = random.nextInt(50);
                int indexParent2 = random.nextInt(50);
                GeneticAlgorithm parent1 = population.get(indexParent1);
                GeneticAlgorithm parent2 = population.get(indexParent2);
                GeneticAlgorithm offspring = parent1.mate(parent2);
    ...
    private static String convertToString(List<Character> chromosome) {
